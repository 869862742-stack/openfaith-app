...
/// Messages Page Golden Tests
/// Covers: messages, messages_friends, messages_groups, messages_rooms
...
void main() {
  group('Messages Page Golden Tests', () {

    testWidgets('Messages - Main tab', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(theme: ThemeData(scaffoldBackgroundColor: AppColors.bgColor), home: _buildMessages(0)));
      await tester.pumpAndSettle();
      await expectLater(find.byType(Scaffold), matchesGoldenFile('goldens/page_messages.png'));
    });

    testWidgets('Messages - Friends tab', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(theme: ThemeData(scaffoldBackgroundColor: AppColors.bgColor), home: _buildMessages(1)));
      await tester.pumpAndSettle();
      await expectLater(find.byType(Scaffold), matchesGoldenFile('goldens/page_messages_friends.png'));
    });

    testWidgets('Messages - Groups tab', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(theme: ThemeData(scaffoldBackgroundColor: AppColors.bgColor), home: _buildMessages(2)));
      await tester.pumpAndSettle();
      await expectLater(find.byType(Scaffold), matchesGoldenFile('goldens/page_messages_groups.png'));
    });

    testWidgets('Messages - Rooms tab', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(theme: ThemeData(scaffoldBackgroundColor: AppColors.bgColor), home: _buildMessages(3)));
      await tester.pumpAndSettle();
      await expectLater(find.byType(Scaffold), matchesGoldenFile('goldens/page_messages_rooms.png'));
    });
  });
}

Widget _buildMessages(int tab) {
  return Scaffold(
    backgroundColor: AppColors.bgColor,
    body: Column(children: [
      _buildGlassHeader('消息'),
      _buildTabBar(['消息', '好友', '群组', '共修'], tab),
      Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
        if (tab == 0) ...[
          _buildMessageItem('张三', '你好，今天的祷告很有力量', '2分钟前', 'Z', [AppColors.auroraRed, AppColors.auroraOrange], unread: 2),
          const SizedBox(height: 8),
          _buildMessageItem('李明', '一起参加明天的读书会吧', '1小时前', 'L', [AppColors.auroraBlue, AppColors.auroraPurple]),
          const SizedBox(height: 8),
          _buildMessageItem('圣经研读群', '王芳: 分享一段经文给大家', '3小时前', 'B', [AppColors.auroraGreen, AppColors.auroraCyan], unread: 5),
          const SizedBox(height: 8),
          _buildMessageItem('晨间冥想房', '系统: 新的共修已开始', '昨天', 'C', [AppColors.auroraYellow, AppColors.auroraGreen]),
        ],
        if (tab == 1) ...[
          _buildSearchBar(),
          const SizedBox(height: 16),
          const Text('好友请求', style: TextStyle(color: AppColors.textPrimary, fontSize: 14, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          _buildFriendRequest('赵六', '希望能认识你', 'add'),
          const SizedBox(height: 8),
          _buildFriendRequest('钱七', '一起共修过', 'pending'),
          const SizedBox(height: 16),
          const Text('我的好友', style: TextStyle(color: AppColors.textPrimary, fontSize: 14, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          _buildFriendItem('张三', '基督教', '在线', 'Z', [AppColors.auroraRed, AppColors.auroraOrange]),
          const SizedBox(height: 8),
          _buildFriendItem('李明', '佛教', '3小时前', 'L', [AppColors.auroraBlue, AppColors.auroraPurple]),
        ],
        if (tab == 2) ...[
          _buildSearchBar(),
          const SizedBox(height: 16),
          _buildGroupItem('圣经研读群', '25人', '今日活跃', 'B', [AppColors.auroraGreen, AppColors.auroraCyan]),
          const SizedBox(height: 8),
          _buildGroupItem('冥想共修团', '18人', '5人在线', 'M', [AppColors.auroraPurple, AppColors.auroraBlue]),
          const SizedBox(height: 8),
          _buildGroupItem('信仰交流', '42人', '今日活跃', 'F', [AppColors.auroraRed, AppColors.auroraOrange]),
        ],
        if (tab == 3) ...[
          _buildRoomCard('晨间冥想', '5人正在共修', Icons.self_improvement),
          const SizedBox(height: 8),
          _buildRoomCard('深夜祷告', '3人正在共修', Icons.nightlight_round),
          const SizedBox(height: 8),
          _buildRoomCard('经文朗读', '8人正在共修', Icons.menu_book),
        ],
      ]))),
    ]),
...
Widget _buildMessageItem(String name, String lastMsg, String time, String letter, List<Color> gradient, {int unread = 0}) {
  return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.cardBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.borderColor, width: 0.5)), child: Row(children: [
    Stack(children: [
      Container(width: 48, height: 48, decoration: BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: gradient)), child: Center(child: Text(letter, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)))),
      if (unread > 0) Positioned(right: -2, top: -2, child: Container(width: 18, height: 18, decoration: const BoxDecoration(shape: BoxShape.circle, color: AppColors.error), child: Center(child: Text('$unread', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold))))),
    ]),
    const SizedBox(width: 12),
...
Widget _buildFriendRequest(String name, String msg, String status) {
  return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.cardBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.borderColor, width: 0.5)), child: Row(children: [
    Container(width: 44, height: 44, decoration: BoxDecoration(shape: BoxShape.circle, gradient: AppColors.auroraGradientWithOpacity(0.3)), child: const Center(child: Icon(Icons.person, color: Colors.white, size: 22))),
    const SizedBox(width: 12),
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
