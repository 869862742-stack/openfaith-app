...
class _LearnScreenState extends State<LearnScreen> with TickerProviderStateMixin {
  ...
  Widget _buildHeader() {
    ...
    return Container(color: AppColors.background, child: Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(16, 12, 16, 8), child: Container(height: 40, decoration: BoxDecoration(color: const Color(0xFF161B22), borderRadius: BorderRadius.circular(12)),
      ...
      Container(margin: const EdgeInsets.symmetric(horizontal: 16), child: TabBar(controller: _tabController, indicator: const BoxDecoration(), indicatorSize: TabBarIndicatorSize.label, labelPadding: EdgeInsets.zero, tabAlignment: TabAlignment.fill,
  ...
  Widget _buildLibraryTab() {
    ...
    return Column(children: [
      Container(margin: const EdgeInsets.symmetric(horizontal: 16), child: TabBar(controller: _libraryTabController, indicator: const BoxDecoration(), indicatorSize: TabBarIndicatorSize.label, labelPadding: EdgeInsets.zero, tabAlignment: TabAlignment.fill,
        tabs: [_buildLibSubTab('Books', Icons.library_books, 0), _buildLibSubTab('Shelf', Icons.bookmark, 1), _buildLibSubTab('Notes', Icons.edit_note, 2), _buildLibSubTab('History', Icons.history, 3)])),
      const SizedBox(height: 8),
  ...
  Widget _buildLibSubTab(String label, IconData icon, int index) {
    return Tab(child: Builder(builder: (context) {
      ...
      return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), color: active ? const Color(0x14FFFFFF) : Colors.transparent),
        child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 12, color: active ? Colors.white : const Color(0xFF8B949E)), const SizedBox(width: 4),
  ...
  Widget _buildClassicsSubTab() {
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...topLevel.map((g) {
        ...
        return Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: const Color(0xFF0D1117), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFF1F2937))),
          child: ListTile(leading: Container(width: 48, height: 48, decoration: BoxDecoration(color: const Color(0xFF161B22), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.library_books, color: Color(0xB3FFFFFF), size: 24)),
            ...
            subtitle: Text([if (children.isNotEmpty) '${children.length} sub', if (children.isNotEmpty && books.isNotEmpty) ' \u00b7 ', if (books.isNotEmpty) '${books.length} vol'].join(), style: const TextStyle(color: Color(0xFF8B949E), fontSize: 12)),
      ...
      if (ungrouped.isNotEmpty) ...[const SizedBox(height: 8), const Text('Other Classics', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)), const SizedBox(height: 12), ...ungrouped.map((b) => _buildBookCard(b))],
  ...
  Widget _buildShelfSubTab() {
    if (_bookmarks.isEmpty) return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.bookmark_border, color: Color(0x4DFFFFFF), size: 48), SizedBox(height: 12), Text('Shelf empty', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 14)), SizedBox(height: 4), Text('Add from Library', style: TextStyle(color: Color(0x4DFFFFFF), fontSize: 12))]));
    final bBooks = _books.where((b) => _bookmarks.contains(b.id)).toList();
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('My Shelf (${bBooks.length})', style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w600)), const SizedBox(height: 12),
      ...bBooks.map((book) {
  ...
  Widget _buildNotesSubTab() {
    if (_notes.isEmpty) return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.edit_note, color: Color(0x4DFFFFFF), size: 48), SizedBox(height: 12), Text('No notes', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 14))]));
  ...
  Widget _buildHistorySubTab() {
    if (_readingHistory.isEmpty) return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.history, color: Color(0x4DFFFFFF), size: 48), SizedBox(height: 12), Text('No history', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 14))]));
    return ListView.builder(padding: const EdgeInsets.all(16), itemCount: _readingHistory.length, itemBuilder: (c, i) {
      ...
      return GestureDetector(onTap: () {
        ...
        child: Row(children: [Container(width: 40, height: 40, decoration: BoxDecoration(color: const Color(0xFF161B22), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.menu_book, color: Color(0xB3FFFFFF), size: 20)),
          const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(bTitle, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis), const SizedBox(height: 2), Text('Reading: $cTitle', style: const TextStyle(color: Color(0x80FFFFFF), fontSize: 12)), if (ts.isNotEmpty) Text(_formatTs(ts), style: const TextStyle(color: Color(0x4DFFFFFF), fontSize: 11))])),
          const Icon(Icons.chevron_right, color: Color(0x4DFFFFFF), size: 20)])));
    });
  }
  ...
  String _formatTs(String iso) {
    try { final dt = DateTime.parse(iso); final diff = DateTime.now().difference(dt);
      if (diff.inMinutes < 1) return 'Now'; if (diff.inHours < 1) return '${diff.inMinutes}m'; if (diff.inDays < 1) return '${diff.inHours}h'; if (diff.inDays < 7) return '${diff.inDays}d'; return '${dt.month}/${dt.day}';
    } catch (_) { return ''; }
  }

  List<BookGroup> _getTopLevelGroups() { final cids = <String>{}; for (final g in _groups) { if (g.groupIds.isNotEmpty) cids.addAll(g.groupIds); } return _groups.where((g) => (g.groupIds.isNotEmpty && g.isPublished) || (!cids.contains(g.id) && g.isPublished)).toList(); }
  List<BookGroup> _getChildGroups(String pid) { final p = _groups.where((g) => g.id == pid).firstOrNull; if (p == null || p.groupIds.isEmpty) return []; return _groups.where((g) => p.groupIds.contains(g.id) && g.isPublished).toList(); }
  List<BookItem> _getGroupBooks(String gid) { final g = _groups.where((gr) => gr.id == gid).firstOrNull; final ids = (g?.bookIds ?? []).toSet(); return _books.where((b) => ids.contains(b.id) || b.groupId == gid).toList(); }
  List<BookItem> _getUngroupedBooks() { if (_groups.isEmpty) return _books; final ids = <String>{}; for (final g in _groups) { ids.addAll(g.bookIds); } return _books.where((b) => (b.status == 'published' || b.status == null || b.status == '') && !ids.contains(b.id) && (b.groupId == null || b.groupId == '')).toList(); }

  void _navigateIntoGroup(BookGroup group) { Navigator.push(context, MaterialPageRoute(builder: (_) => GroupDetailScreen(group: group, allGroups: _groups, allBooks: _books, chaptersMap: _chaptersMap))); }
    ...
    return GestureDetector(onTap: () { Navigator.push(context, MaterialPageRoute(builder: (_) => BookDetailScreen(bookId: book.id, bookTitle: book.title, bookReligion: book.religion, bookCategory: book.category, bookDescription: book.description))); },
      child: Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: const Color(0xFF0D1117), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFF1F2937))),
        child: ListTile(leading: Container(width: 48, height: 48, decoration: BoxDecoration(color: const Color(0xFF161B22), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.menu_book, color: Colors.white, size: 24)),
          ...
          subtitle: Text([book.category.isNotEmpty ? book.category : book.religion, if (_chaptersMap.containsKey(book.id)) ' \u00b7 ${_chaptersMap[book.id]} ch'].join(), style: const TextStyle(color: Color(0xFF8B949E), fontSize: 12)),
  ...
  Widget _buildCalendarTab() {
    ...
    const wk = ['Su','Mo','Tu','We','Th','Fr','Sa']; const mn = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 8), const Text('This Month', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w600)), const SizedBox(height: 12),
      if (_monthHolidays.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(16), child: Text('No holidays', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 13)))) else ..._monthHolidays.map((h) => _buildHolidayListItem(h)),
    ]));
  }
  ...
  Widget _buildSelectedDateHolidays() {
    ...
    return Container(margin: const EdgeInsets.only(bottom: 16), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0x08FFFFFF), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0x14FFFFFF))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('${_selectedDate!.month}/${_selectedDate!.day}', style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w500)), const SizedBox(height: 8),
        if (holidays.isEmpty) const Text('None today', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 13)) else ...holidays.map((h) => Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [Container(width: 8, height: 8, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFFFF4D6D), Color(0xFFFF9F1C), Color(0xFFFFD60A), Color(0xFF70E000), Color(0xFF00E5FF)]))), const SizedBox(width: 8), Text(h.name, style: const TextStyle(color: Colors.white, fontSize: 13)), const SizedBox(width: 6), Text('(${h.religion})', style: const TextStyle(color: Color(0x80FFFFFF), fontSize: 12))]))),
      ]));
  }
  ...
  void _showYearPicker() {
    ...
    showDialog(context: context, builder: (ctx) => Dialog(backgroundColor: const Color(0xFF0D1117), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Container(width: 200, padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('Year', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)), const SizedBox(height: 16),
  ...
  void _showMonthPicker() {
    ...
    showDialog(context: context, builder: (ctx) => Dialog(backgroundColor: const Color(0xFF0D1117), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Container(width: 200, padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('Month', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)), const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: AppColors.background,
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (groupBooks.isNotEmpty) ...[if (childGroups.isNotEmpty) ...[const SizedBox(height: 8), const Text('Books', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)), const SizedBox(height: 12)],
